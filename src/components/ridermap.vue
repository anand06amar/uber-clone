...
onMounted(async () => {
    ...
    const dropoffLocationName = route.query.dropofflocation;
    console.log('Pickup Location Name:', pickupLocationName);
    console.log('Pickup Location Name:', dropoffLocationName);


    const [pickupLongitude, pickupLatitude] = await geocodeLocation(pickupLocationName);
    const [dropoffLongitude, dropoffLatitude] = await geocodeLocation(dropoffLocationName);
    ...
    if (dropoffLongitude && dropoffLatitude) {
        new mapboxgl.Marker().setLngLat([dropoffLongitude, dropoffLatitude]).addTo(map);
    }
});
